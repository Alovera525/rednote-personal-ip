...
| 步骤 | 做什么 | 关键判断 |
|---|---|---|
| 0 · 建立创作卡 | 新号确认 IP 风格与文章口气；老号导入爆帖 | 风格先确认，格式首次由用户选择 |
...
首次完整创作会先识别账号阶段。**从零起号**时，Skill 按行业 / 身份展示 3 个相关个人 IP 风格候选，让用户确认个人 IP 风格、文章口气和交付格式，再写文案；用户也可提供 1–3 个参考账号，Skill 只借鉴定位、选题支柱、栏目与结构，并将它们转化为用户自己的方向，不复制声音、原文或个人经历。确认后可建立长期档案。**已有在运营账号**时，用户可粘贴 1–3 篇自己的或已获授权的爆帖，Skill 会提炼结构、语气、用词、微习惯与禁用项，回显“账号风格确认卡”后再创作。不会复制原文，也不会把一条高数据内容直接当成长久风格。
...
完整笔记首次开始前，用户必须选择 Word 友好版、Markdown（`.md`）或纯文本（`.txt`）；不会默认 Word。三者都是聊天中的最终内容格式：Word 友好版可直接粘贴进 Word，Markdown 和纯文本可由用户自行保存；插件保持纯 Markdown，不附带程序或文件导出器。
...
```text
@rednote-personal-ip 我是学生博主，从零起号。先让我选择个人 IP 风格、文章口气和 .md 交付，再写标题、正文和 hashtags，不生成图片
