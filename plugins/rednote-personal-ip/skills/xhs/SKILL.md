...
- 要标题和内容或“做一条笔记” → 先展示一张快速创作卡，用户回复“按推荐写”或修改其中一项后，再交付标题、正文、hashtags 和置顶评论；随后询问是否需要制作封面，收到选择后继续发布前自检。
...
新账号第一次生成完整笔记时，先按 `xhs-post` 第 0 步展示一张快速创作卡：根据行业 / 身份推荐一个暂定 IP 风格、本篇表达方式和两个风格落点。不要只让用户交 3–5 段微信，也不要先抛出 4–6 张风格卡；用户主动要深度建档或看选项时才转 `xhs-voice`。输入空泛时用结构化的真实场景问题补强，不替用户编细节。
