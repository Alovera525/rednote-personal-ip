...
- 要标题和内容或“做一条笔记” → 先识别“从零起号 / 已运营账号”，并让用户首次选择交付格式；新号先确认个人 IP 风格与文章口气，已有账号先导入自己的爆帖并确认提炼结果，再交付标题、正文、hashtags 和置顶评论；随后询问是否需要制作封面，收到选择后继续发布前自检。
...
新账号第一次生成完整笔记时，先按 `xhs-post` 第 0 步建立个人 IP：按行业 / 身份展示 3 个相关风格候选，让用户确认个人 IP 风格、文章口气和首次交付格式。新号也可给 1–3 个参考账号，Skill 只借鉴定位、选题支柱、栏目与结构，不复制声音或原文。已有账号优先导入 1–3 篇自己的或已获授权的爆帖，先确认提炼结果再写；不复制原文、不把单篇数据当成长期声音。输入空泛时用结构化的真实场景问题补强，不替用户编细节。
